<template>
      <div class="flex-col items-center w-full mt-[212px] mb-[249px] lg:mt-[142px] lg:mb-[199px] px-4">
        <img
          src="/images/icon/icon_chat.png"
          alt="logo"
          class="mb-[44px]"
        />
        <h2 class="text-[40px] font-bold  md:text-[30px]">복잡한 인터넷 문자 서비스를</h2>
        <h2 class="text-[40px] font-bold  md:text-[30px]">간편하게 만나보세요.</h2>
        <h3 class="mt-[24px] text-xl text-[#5c5c5c] font-semibold">모든 종류의 인터넷 SMS를 편리하게.</h3>
        <div class="flex justify-between w-full mt-24 xl:justify-center gap-4 lg:flex-col items-center lg:gap-6">
          <div class="flex-col items-center justify-center w-[444px] h-[349px] rounded-2xl bg-[#f0f0fa] gap-[22px] xl:w-[335px] lg:w-3/4 md:w-full">
            <img src="/images/icon/icon_write.png"/>
            <div class="flex-col items-center gap-1">
              <h2 class="text-xl font-bold xl:text-[18px] lg:text-[25px] md:text-lg">메시지 용량에</h2>
              <h2 class="text-xl font-bold xl:text-[18px] lg:text-[25px] md:text-lg">제한 받지 않고 전송</h2>
            </div>
            <div class="flex-col items-center gap-1">
              <h3 class="text-base xl:text-[14px] lg:text-xl md:text-[12px]">메시지 용량 걱정없이 단문 메세지는</h3>
              <h3 class="text-base xl:text-[14px] lg:text-xl md:text-[12px]">물론 장문, 사진까지 전송이 가능합니다</h3>
            </div>
          </div>
          <div class="flex-col items-center justify-center w-[444px] h-[349px] rounded-2xl bg-[#f0f0fa] gap-[22px] xl:w-[335px] lg:w-3/4 md:w-full">
            <img src="/images/icon/icon_mail.png" />
            <div class="flex-col items-center gap-1">
              <h2 class="text-xl font-bold xl:text-[18px] lg:text-[25px] md:text-lg">모든 종류의</h2>
              <h2 class="text-xl font-bold xl:text-[18px] lg:text-[25px] md:text-lg">문자 메시지를 한번에</h2>
            </div>
            <div class="flex-col items-center gap-1">
              <h3 class="text-base xl:text-[14px] lg:text-xl md:text-[12px]">메시지 용량 걱정없이 단문 메세지는</h3>
              <h3 class="text-base xl:text-[14px] lg:text-xl md:text-[12px]">물론 장문, 사진까지 전송이 가능합니다</h3>
            </div>
          </div>
          <div class="flex-col items-center justify-center w-[444px] h-[349px] rounded-2xl bg-[#f0f0fa] gap-[22px] xl:w-[335px] lg:w-3/4 md:w-full">
            <img src="/images/icon/logo_speaker.png" />
            <div class="flex-col items-center gap-1">
              <h2 class="text-xl font-bold xl:text-[18px] lg:text-[25px] md:text-lg">간편한 광고</h2>
              <h2 class="text-xl font-bold xl:text-[18px] lg:text-[25px] md:text-lg">문자 메시지 설정</h2>
            </div>
            <div class="flex-col items-center gap-1">
              <h3 class="text-base xl:text-[14px] lg:text-xl md:text-[12px]">광고 설정 버튼 클릭 한번으로</h3>
              <h3 class="text-base xl:text-[14px] lg:text-xl md:text-[12px]">080 수신 거부 번호가 적용됩니다.</h3>
            </div>
          </div>
        </div>
      </div>
</template>

<script>


    export default {
        name:"MainSection1",
    }
</script>

<style scoped>
</style>